...
/* maybe LIST_POISON1 and LIST_POISON2 should be 0 */
...
extern bool __list_add_valid(struct list_head *lnew,
			      struct list_head *prev,
			      struct list_head *next);
extern bool __list_del_entry_valid(struct list_head *entry);
#else
static inline bool __list_add_valid(__attribute__((unused))struct list_head *lnew,
...
static inline void __list_add(struct list_head *lnew,
			      ...
			      struct list_head *next)
{
	if (!__list_add_valid(lnew, prev, next))
		return;

	next->prev = lnew;
	lnew->next = next;
	lnew->prev = prev;
	WRITE_ONCE(prev->next, lnew);
}
