/* a simple hash list with nessasery functions */
#ifndef __HASH_H__
#define __HASH_H__
...
struct ace_hash {
	...
	unsigned int n_elem_all;
	unsigned int n_memory;
	unsigned int n_bucket;
	struct ace_hash_head ha_head[0];
};

struct ace_hash_elem {
...
static struct ace_hash *ace_hash_create(unsigned int n_bucket, unsigned int max_elem)
{
	unsigned int size = sizeof(struct ace_hash) + sizeof(struct ace_hash_head) * n_bucket;
	struct ace_hash *h = (struct ace_hash*)malloc(size);
	if (!h) {
		return NULL;
	}
	memset(h, 0, size);
	...
	for (unsigned int i = 0; i < n_bucket; i++) {
		struct hlist_head *hh = &h->ha_head[i].hl_head;
		INIT_HLIST_HEAD(hh);
	}
...
static int ace_hash_add(struct ace_hash *h, char *k, unsigned int kl, char *v, unsigned int vl)
{
	unsigned int c = 0;

	for (unsigned int i = 0; i < kl; i++) {
		c ^= crc32c_1byte(k[i], c);
	}

	struct ace_hash_elem *elem = (struct ace_hash_elem*)malloc(sizeof(struct ace_hash_elem));
	if (!elem) {
		return -1;
	}
	...
	elem->vlen = vl;
	elem->sig = c;
	INIT_HLIST_NODE(&elem->hl_node);

	struct ace_hash_head *ha_head = &h->ha_head[c % h->n_bucket];
	struct hlist_head *hh = &ha_head->hl_head;

	hlist_add_head(&elem->hl_node, hh);
...
static int ace_hash_lookup(struct ace_hash *h, char *k, unsigned int kl, char **v, unsigned int *vl)
{
	unsigned int c = 0;
...
static int ace_hash_del(struct ace_hash *h, char *k, unsigned int kl, char **v, unsigned int *vl)
{
	unsigned int c = 0;
...
static unsigned int ace_hash_count(struct ace_hash *h)
{
	unsigned int c = 0;
...
static void ace_hash_free(struct ace_hash *h, int (*func)(void *k, void *v))
{
	for (unsigned int i = 0; i < h->n_bucket; i++) {
		struct hlist_head *hh = &h->ha_head[i].hl_head;
		struct ace_hash_elem *elem = NULL;
		struct hlist_node *n = NULL;
		hlist_for_each_entry_safe(elem, n, hh, hl_node) {
			hlist_del(&elem->hl_node);
			if (func) {
				func(elem->key, elem->val);
			}
			free(elem);
			h->n_elem_all--;
		}
